...
# borrowed from the Interaction Dataset codebase
...
def batched_rotate_around_center(pts, center, yaw):
    """
    Rotate many points around center.
    Args:
        pts (np.array): (*,4,2) corners
        center (np.array): (*,2) center points
        yaw (np.array): (*,) yaws
    Returns:
        rotated (np.array): (*,4,2) corners
    """

    center = np.expand_dims(center, axis=1)
    R = np.stack([np.stack([np.cos(yaw), np.sin(yaw)], axis=-1),
                  np.stack([-np.sin(yaw), np.cos(yaw)], axis=-1)], axis=1)
    rotated = (pts-center) @ R + center
